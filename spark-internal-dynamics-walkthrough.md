...
|----------------------------------------------------------------------------| ------------------------------------------ | -------------------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------- | -------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| **Identify Slow / Stale Jobs**                                             | `yarn application -list -appStates RUNNING | awk '$5 > 7200000'`                                | List running jobs exceeding 2 hours                                                  | `# Checks jobs running more than 2 hours; helps find slow/stale jobs` |                                  |                                                                      |                                                                                |
|                                                                            | `yarn application -list                    | grep -v "churn"                                    | grep "default"                                                                       | grep "RUNNING"                                                        | awk '{print $1}' > job_list.txt` | List running jobs in a specific queue                                | `# Generates job IDs of long-running jobs in default queue; ignore churn jobs` |
|                                                                            | ResourceManager / Application Master UI    | Monitor running jobs, stages, and executor usage   | `# Use RM UI at http://<rm_host>:8088 to see resource usage and job states`          |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Spark UI                                   | Visualize DAG, stages, tasks, and shuffle          | `# Spark UI shows stage runtime, task distribution, shuffle size, and stragglers`    |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Oozie / Airflow Console                    | Track workflow-level progress and stuck tasks      | `# Check for stuck or pending tasks in orchestration pipelines`                      |                                                                       |                                  |                                                                      |                                                                                |
| **Track Job Failures**                                                     | `yarn application -list 2>/dev/null        | grep "<queue_name>"                                | grep "FAILED"                                                                        | awk '{print $1}' > job_list.txt`                                      | List failed jobs per queue       | `# Collect application IDs of failed jobs for further investigation` |                                                                                |
|                                                                            | `yarn logs -applicationId <app_id>`        | View application master logs                       | `# Use this to read logs and understand why a job failed`                            |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Spark History Server                       | Review completed job details and metrics           | `# Check stage-level metrics and failed tasks for completed jobs`                    |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Airflow / Oozie logs                       | Check task-level failure details                   | `# Identify exactly which task or workflow step caused failure`                      |                                                                       |                                  |                                                                      |                                                                                |
| **Optimize Slow Spark Jobs (Slow in Multi-node vs faster in Single-node)** | Executor Configuration                     | Tune number of executors, cores, and memory        | `# Too many small executors can slow jobs; align executor size with data volume`     |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Partitioning / Shuffling                   | Adjust `spark.sql.shuffle.partitions`              | `# Reduces shuffle overhead; too many partitions can increase task scheduling time`  |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Task Parallelism                           | Ensure even data distribution; use broadcast joins | `# Skewed partitions cause straggler tasks; broadcast small tables to avoid shuffle` |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Caching / Persisting                       | Cache intermediate DataFrames                      | `# Reuse the same DataFrame multiple times without recomputation`                    |                                                                       |                                  |                                                                      |                                                                                |
|                                                                            | Spark UI Monitoring                        | Inspect stage execution times, shuffle read/write  | `# Identify which stage takes longest and optimize transformations accordingly`      |                                                                       |                                  |                                                                      |                                                                                |

**Summary Comments:**

* CLI commands are **quick for spotting slow or failed jobs**.
* UIs (Spark, RM, AM, Airflow/Oozie) provide **detailed diagnostics and visualization**.
* Optimizations focus on **resources, partitions, shuffle, caching, and skew handling**.
* Following these steps ensures **efficient cluster usage, faster jobs, and easier debugging**.

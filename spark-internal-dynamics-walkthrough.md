...
| **Category**                                             | **Method / Command**                       | **Purpose / Details**                              | **Notes / Best Practices**                             |                                                |                                  |                                        |                                   |
| -------------------------------------------------------- | ------------------------------------------ | -------------------------------------------------- | ------------------------------------------------------ | ---------------------------------------------- | -------------------------------- | -------------------------------------- | --------------------------------- |
| **Identify Slow / Stale Jobs**                           | `yarn application -list -appStates RUNNING | awk '$5 > 7200000'`                                | List running jobs exceeding 2 hours                    | Helps detect jobs running longer than expected |                                  |                                        |                                   |
|                                                          | `yarn application -list                    | grep -v "churn"                                    | grep "default"                                         | grep "RUNNING"                                 | awk '{print $1}' > job_list.txt` | List running jobs in a specific queue  | Useful for queue-level monitoring |
|                                                          | ResourceManager / Application Master UI    | Monitor running jobs, stages, and executor usage   | Provides visual insights into slow jobs                |                                                |                                  |                                        |                                   |
|                                                          | Spark UI                                   | Visualize DAG, stages, tasks, and shuffle          | Detect bottlenecks in stages or tasks                  |                                                |                                  |                                        |                                   |
|                                                          | Oozie / Airflow Console                    | Track workflow-level progress and stuck tasks      | Identifies jobs blocked in orchestration pipelines     |                                                |                                  |                                        |                                   |
| **Track Job Failures**                                   | `yarn application -list 2>/dev/null        | grep "<queue_name>"                                | grep "FAILED"                                          | awk '{print $1}' > job_list.txt`               | List failed jobs per queue       | Provides app IDs for log investigation |                                   |
|                                                          | `yarn logs -applicationId <app_id>`        | View application master logs                       | Helps find the root cause of failure                   |                                                |                                  |                                        |                                   |
|                                                          | Spark History Server                       | Review completed job details and metrics           | Useful for batch jobs post-mortem analysis             |                                                |                                  |                                        |                                   |
|                                                          | Airflow / Oozie logs                       | Check task-level failure details                   | Useful for workflow-driven pipelines                   |                                                |                                  |                                        |                                   |
| **Optimize Slow Spark Jobs (Multi-node vs Single-node)** | Executor Configuration                     | Tune number of executors, cores, and memory        | Avoid too many small executors; align with data volume |                                                |                                  |                                        |                                   |
|                                                          | Partitioning / Shuffling                   | Adjust `spark.sql.shuffle.partitions`              | Reduce shuffle overhead and balance tasks              |                                                |                                  |                                        |                                   |
|                                                          | Task Parallelism                           | Ensure even data distribution; use broadcast joins | Prevent straggler tasks due to data skew               |                                                |                                  |                                        |                                   |
|                                                          | Caching / Persisting                       | Cache intermediate DataFrames                      | Avoid recomputation of reused data                     |                                                |                                  |                                        |                                   |
|                                                          | Spark UI Monitoring                        | Inspect stage execution times, shuffle read/write  | Identify bottlenecks and optimize transformations      |                                                |                                  |                                        |                                   |

...
## 47. How to change column type in RDD, DataFrame, and Spark SQL?

| Type                    | Method                          | Example                                            |
| ----------------------- | ------------------------------- | -------------------------------------------------- |
| **RDD**                 | Use `map()` and cast manually   | `rdd.map(lambda x: (x[0], int(x[1])))`             |
| **DataFrame (PySpark)** | Use `withColumn()` and `cast()` | `df = df.withColumn("age", df["age"].cast("int"))` |
| **Spark SQL**           | Use `CAST()` in SQL             | `spark.sql("SELECT CAST(age AS INT) FROM people")` |


---

## 48. Have you handled date functions in Spark? How to extract the year from a given date?

✅ **Ans:** Yes, Spark provides built-in date functions in `pyspark.sql.functions`.
...
df = df.withColumn("year", year(to_date(df["date_col"], "yyyy-MM-dd")))
df.show()
```
...
```sql
SELECT year(to_date(date_col, 'yyyy-MM-dd')) AS year FROM table;
```
---
